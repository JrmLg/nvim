...
local mySettings = require("neotree.mySettings")
...
local m = {
    ...
    system_execute = function(state)
        ...
        if path ~= "" then
            path, _ = path:gsub("\\", "\\\\")
            vim.fn.RunScript("system-execute", path)
        end
    end,
